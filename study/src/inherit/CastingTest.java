...
public class CastingTest {
    ...
    //<--> 오버라이딩(override=overwrite, 덮어쓰다)
    //상속 부모클래스(run() {구현내용} )
    //         <-- 자식 클래스(run() {구현내용 재정의})

    //많은 양의 데이터를 모으고 싶음.(가변, ex)게시판 )
    //다양한 데이터 타입을 모으고 싶음.
    //배열 - 고정길이, 같은 데이터타입을 모을 때 사용
    //부품 - 종류가 엄청 많음
    //배열은 순서를 가지고 있음.(인덱스)
    ...
    //--> 인터페이스 List
    //--> 인터페이스에 정의된 기능을 그대로 클래스로 만들어놓음
    //ArrayList, LinkedList, ...

    //같은 패키지내에 있는 것은 new해서 사용할 수 있지만
    //다른 패키지에 있으면 위치를 알려줘야한다!
    //import java.util.*!!!!!!!!!!!!!
    ArrayList list = new ArrayList(); //가변길이, 다양한 타입을 넣을 수 있음
    //List list = new LinkedList();
    //List list --> List인터페이스에 써넣은 대로 구현한 클래스는 다 집어넣을 수 있음
    //자동 형변환
    //ArrayList list2 = new LinkedList(); // arraylist와 linkedlist는 형제관계라 대입 불가능
    ...
    //변수 언제쓰나 ? (변수를 쓸 때 형변환이 이루어짐)
    //: 1)클래스의 필드, 2)매개변수를 쓸 때 형변환을 많이 씀.

        System.out.println(list); //리스트에 들어있는 값들
        //강제형변환이 필요한가 ?
        //add()할 때 object 타입으로 자동형변환되어 들어감.
        //꺼낼때도 Object타입으로 꺼내온다.
        Object o = list.get(0); //list에 첫번째
        //Object클래스는 8개의 메서드
        //ABC, toLowerCase() 소문자로, charAt() 한글자 추출
        //o.charAt(); //object에 charAt 없기때문에 오류남. object타입에는 string의 메서드를 호출 불가능
        //꺼내서 String의 메서드를 쓰고 싶은 경우는 다시 String으로
