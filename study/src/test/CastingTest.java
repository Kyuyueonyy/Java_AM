...
public class CastingTest {
    public static void main(String[] args) {
        ...
        byte age3 = (byte)age2; //age3은 1바이트, age2는 4바이트 이므로 강제로 형변환을 해줘야함

        //기본형끼리의 타입 변환은 큰지 작은지만 판단하면 가능
        ...
        int num = Integer.parseInt(data);  //기본형<-참조형 대입하려면 대입연산자로 형변환 불가능
        //타입이 다를다면 부품을 이용해야함!
        //Integer.parseInt() 부품
        //정수타입별 모두 있음.(ppt 52)

        System.out.println("내년 나이는 " + (num+1));

        String data2 = "11.11";
        double num2 = Double.parseDouble(data2); //Float.parseFloat()도 있음.
        double num3 = Double.parseDouble("11.33"); // 11.33

        //숫자를 문자열로 바꾸고 싶은 경우(=로 불가능함)
